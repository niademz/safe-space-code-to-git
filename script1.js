...
function loadChatHistory() {
  // Define the URL of your deployed Netlify Function
  const chatHistoryUrl = 'https://safe-space-teens.netlify.app/.netlify/functions/loadChatHistory'; // Adjust based on your function's URL

  // Make an HTTP GET request to the function's URL with query parameters (optional)
  fetch(chatHistoryUrl, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
    // Add query parameters if applicable (e.g., for chatroom name)
    // queryParams: { chatroom: chatroom }, // Optional
  })
  .then(response => response.json())
  .then(data => {
    // Handle successful response with chat history data
    console.log('Chat history:', data);

    // Update the UI with the received chat history (modify based on your UI structure)
